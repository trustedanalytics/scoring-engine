...
class scorer(object):
    ...
    def __init__(self, port_id, host=config.scoring_engine_host):
        ...
        #filepath = os.path.abspath(os.path.join(
            #config.root, "regression-tests", "lib", "port.ini"))
    ...
    def __enter__(self):
        ...
        run_path = os.path.abspath(os.path.join(config.root, "scoring_engine", "model-scoring-core"))

        # keep track of cwd for future
        test_dir = os.getcwd()
        os.chdir(run_path)
        print "full command: " + str("./bin/start-server.sh" + "-Dtrustedanalytics.scoring-engine.archive-mar=" + self.hdfs_path + 
